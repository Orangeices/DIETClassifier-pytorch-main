    ...
    def __init__(self, config: DIETClassifierConfig):
        ...
        if path.exists(config.model):
            try:
                ...
                json_config.update({"model": config.model, "entities": config.entities, "intents": config.intents})
            ...
            pretrained_model.config.update(
                {"model": config.model, "entities": config.entities, "intents": config.intents})
...
if __name__ == '__main__':
    ...
    config = DIETClassifierConfig(
        model=r"F:\BaiduNetdiskDownload\huggingface_models\bert-base-uncased",
        entities=entities_list,
        intents=intents_list
    )
    model = DIETClassifier(config=config)

    sentences = ["How to check attendance?"]

    inputs = tokenizer(sentences, return_tensors="pt", padding="max_length", max_length=10)
